...
//默认的far_c和focal（近平面和远平面，近平面同时也是焦距），返回带far_c和focal，width，height的CameraParameters
...
//	if (isBin) { bs_in.open(bs_file, std::ios::binary); } // 打开bs_file（即mesh文件）
//	else { bs_in.open(bs_file); }
//	if (bs_in.is_open()) 
//	{
//		this->bsVideo_.resize(nFrame);//resize bsvideo的帧数
//		model_bs.resize(NUM_BLENDSHAPE + 1);//resize model_bs个数为52
//		for (int idx_frame = 0; idx_frame < nFrame_; idx_frame++) //对每一帧中的每一个bs，从文件传入model_bs
//		{
//			model_bs[0] = 1.0;
