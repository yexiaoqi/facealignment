...
void Blendshape::LoadBs(std::string bs_folder, int nBs) 
{
	...
	obj_mesh.update_normal(obj_mesh._bufmodel);//xiugai yqy 180425
...
void Blendshape::LoadBsBin(std::string bs_file_bin, int nBs)
{
	...
	// prepare obj meshs
	nVert_in = nVert_x3_in / 3;
	obj_mesh._bufmodel.n_verts_ = nVert_in;
	obj_mesh._bufmodel.n_tri_ = nFaces;
	obj_mesh._bufmodel.position_ = Eigen::MatrixXf(nVert_in, 3);	obj_mesh._bufmodel.position_.setZero();
	obj_mesh._bufmodel.color_ = Eigen::MatrixXf(nVert_in, 3);	obj_mesh._bufmodel.color_.setOnes(); /*this->color_ = this->color_ * 255.f;*/
	obj_mesh._bufmodel.normal_ = Eigen::MatrixXf(nVert_in, 3); obj_mesh._bufmodel.normal_.setZero();
	obj_mesh._bufmodel.tex_coord_ = Eigen::MatrixXf(nVert_in, 2);	obj_mesh._bufmodel.tex_coord_.setZero();
	obj_mesh._bufmodel.tri_list_ = Eigen::MatrixXi(nFaces, 3); obj_mesh._bufmodel.tri_list_.setZero();
	obj_mesh._bufmodel.face_normal_ = Eigen::MatrixXf(nFaces, 3); obj_mesh._bufmodel.face_normal_.setZero();
	// read triangle list
	fin.read(reinterpret_cast<char*>(obj_mesh._bufmodel.tri_list_.data()), sizeof(int)*((int)nFaces) * 3);
	// read vertices
	for (int kk = 0; kk < nBs_in; kk++)
	{
		fin.read(reinterpret_cast<char*>(obj_mesh._bufmodel.position_.data()), sizeof(float)*((int)nVert_x3_in));
		obj_mesh.update_normal(obj_mesh._bufmodel);
		blendshapes_.push_back(obj_mesh);
	}
	fin.close();
	std::cout << "done." << std::endl;
...
void Blendshape::UpdateMesh(std::vector<double> coef, Mesh& recons) ////coef即params_
{
	...
	recons._bufmodel.position_.array() = this->blendshapes_[0]._bufmodel.position_.array() * coef[0];
	for (int kk = 1; kk < coef.size(); kk++)
	{
		coef_f = coef[kk];
		recons._bufmodel.position_.array() += (this->blendshapes_[kk]._bufmodel.position_.array() - this->blendshapes_[0]._bufmodel.position_.array()) * coef_f;
	}
	return;
}
...
void Blendshape::ProjectLandmark(std::vector<int> index3D)
{
	...
	for (int ii = 0; ii < index3D.size(); ii++)
	{
		...
		{
			//position_是一个nVert_in行3列的数据
			pos_i(0) = this->blendshapes_[kk]._bufmodel.position_(index3D[ii], 0);//第index3D[ii]行第0列，因为总数据有很多几万行每个mesh，我们只要landmark对应的那几行
			pos_i(1) = this->blendshapes_[kk]._bufmodel.position_(index3D[ii], 1);
			pos_i(2) = this->blendshapes_[kk]._bufmodel.position_(index3D[ii], 2);
			model_i[kk] = pos_i;
		}
		this->landmarks_model_[ii] = model_i;
	}
